...
function unlock(mnemonic) {
    ...
    ROOT = bitcoin.HDNode.fromSeedBuffer(SEED, bitcoin.networks.mainnet)
    ADDRESS = getAddress()
    ticker = "btc"
...
function validateAddress(address) {
    var re = /^[31][a-km-zA-HJ-NP-Z1-9]{25,34}$/ // // Regular Expression https://stackoverflow.com/questions/21683680/regex-to-match-bitcoin-addresses
...
function send(address, amount, fee) {
    ...
    var data = ROOT.derivePath("m/44'/0'/0'/0/0") // This is the change address
    confirmed_transactions = transactions.txrefs
    var total_amount = 0
    var count_amount = 0
    var inputs = 0
    var utxo = true
    var txb = new bitcoin.TransactionBuilder(bitcoin.networks.mainnet)
    ...
    console.log(pushtx)
    $.post('https://api.blockcypher.com/v1/btc/main/txs/push', JSON.stringify(pushtx)) // push the transaction to blockcypher API
        .then(function(d) {
            console.log(d)
        });
}
